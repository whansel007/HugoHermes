...
### Common Pitfalls
* **Timezones:** Notion API expects ISO 8601 strings. Always use `YYYY-MM-DDTHH:MM:SS+08:00` for SGT to avoid UTC drift where tasks shift by 8 hours or jump to the next day.
* **Property Updates:** Updating a page's date requires the full `properties` object for the date field.
* **CLI Reliability:** Many `ntn` or `notion-cli` npm packages are unreliable ghosts. If CLI commands don't produce clear output, fallback to Python/`requests` for complex property manipulation to ensure JSON consistency.
* **Authentication:** Always use the `NOTION_API_KEY` environment variable; never hardcode it.
